 ...
 * @return bool
 */
function addScoreLog($user_id, $type, $score, $desc = "", $pre_score)
{
    if ($type == 1) {
        $after_score = $pre_score + $score;
    } else {
        $after_score = $pre_score - $score;
    }
    model("UserScoreLog")->save([
        "user_id" => $user_id,
        "type" => $type,
        "score" => $score,
        "desc" => $desc,
...
//增加城主金额记录
function addHeaderMoneyLog($header_id, $type, $pre_amount, $money, $order_no = "")
{
    if (in_array($type, ["1", "5"])) {
        $after_money = $pre_amount + $money;
    } else {
        $after_money = $pre_amount - $money;
    }
    if ($money > 0) {
...
//增加团长佣金记录
function addCommissionLog($leader_id, $type, $pre_amount, $money, $order_no = "")
{
    if (in_array($type, ["1", "5"])) {
        $after_money = $pre_amount + $money;
    } else {
...
function qrcode($data)
{
    ...
    $file_name = base64_encode($data) . ".png";
    is_dir(__UPLOAD__ . "/qrcode") or mkdir(__UPLOAD__ . "/qrcode", "0777");
    if (!file_exists(__UPLOAD__ . "/qrcode/" . $file_name)) {
        \QRcode::png($data, __UPLOAD__ . "/qrcode/" . $file_name, 0, 8, 1);
    }
    return __URL__ . "/upload/qrcode/" . $file_name;
}
 ...
 * @return array
 */
function getMonthDay($time = '', $format = 'Y-m-d')
{
    //30日之前日期
    if ($time == '') {
        $time = strtotime("-30 day");
    } else {
        $time = strtotime($time) - 30 * 86400;
    }
    $date = [];
    for ($i = 0; $i < 30; $i++) {
        $date[] = date($format, $time + $i * 86400);
    }
    return $date;
}
